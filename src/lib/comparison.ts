import { MediaDetails, MediaType, SimpleCredit } from "./interfaces";
...
async function compareLikeTypes(
  ...
  const { cast: firstCast, crew: firstCrew } = await getCredits(
    first.mediaType,
    first.id,
  );
  const { cast: secondCast, crew: secondCrew } = await getCredits(
    second.mediaType,
    second.id,
  );
...
function addCredits(
  ...
  credits.forEach((credit) => {
    ...
    if (existing) {
      ...
      let combinedCredit: CombinedCredit = {
        id: credit.id,
        name: credit.name,
